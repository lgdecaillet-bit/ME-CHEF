...
0. Proyecto Expo, EAS Build desde Windows, hello world en TestFlight, Supabase
   con el esquema.
1. Onboarding (sesión anónima; la cuenta se pide al tocar «Cocinar» y cocinar la
   requiere, decisión #29; dos preguntas opcionales), SQLite cifrado, y **el motor
   determinista completo**. Todo lo que no toca IA va
   primero: es la regla de arquitectura aplicada al orden de trabajo.
2. **La nevera de punta a punta.** Guía de cámara, proxy, dos pasadas de visión,
   mapeo con caché, fusión, despensa básica, overlay táctil, tres recetas por
   cobertura, perecibilidad. No se lanza hasta pasar 95 % de precisión en lo
   marcado como seguro, medido con el harness de 200 fotos.
3. Facturas, lista de mercado calculada, Time to cook.
4. Cocinar acompañado: un paso por pantalla, timers, Live Activity, inactividad.
5. Pipeline de recetas en lote, índice del catálogo, asistente, importador de links.
...
> Rutas relativas a `michef/`. Hasta D1 este archivo vive en `michef-starter/`, pero
> `docs/estado.md`, `docs/bitacora.md`, `docs/decisiones.md` y `.claude/agents/` están
> en `michef/`. Los `docs/` de `michef-starter/` son solo `producto.html` y punteros.
